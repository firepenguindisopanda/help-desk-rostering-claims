...
export function AssistantLayout({ children }: AssistantLayoutProps) {
  ...
  const navigation = [
    { name: "Dashboard", href: routes.assist.index, icon: Ticket },
    { name: "Profile", href: routes.assist.profile, icon: User },
    { name: "Time Tracking", href: "#soon:time", icon: MessageSquare },
    { name: "Schedule Changes", href: "#soon:requests", icon: MessageSquare },
  ];
      ...
      <div className={`fixed inset-0 z-50 lg:hidden ${sidebarOpen ? "block" : "hidden"}`}>
        ...
        <div className="fixed left-0 top-0 h-full w-64 bg-card border-r border-border">
          <div className="flex h-16 items-center justify-between px-6">
            <h1 className="text-xl font-bold text-primary">Schedule Manager</h1>
            <Button
              variant="ghost"
              size="sm"
              onClick={() => setSidebarOpen(false)}
          ...
          <nav className="px-4 space-y-2">
            {navigation.map((item) => (
              <Link
                ...
                onClick={() => {
                  if (item.href.startsWith("#soon")) { toast.info("Coming soon"); return; }
          ...
          <nav className="flex-1 px-4 space-y-2 pb-4">
            {navigation.map((item) => (
              <Link
                ...
                onClick={(e) => {
                  if (item.href.startsWith("#soon")) { e.preventDefault(); toast.info("Coming soon"); }
                }}
              >
                <item.icon className="mr-3 h-4 w-4" />
                {item.name}
            ...
            <div className="flex items-center gap-x-4 lg:gap-x-6">
              ...
              <div className="flex items-center gap-x-2">
                <div className="hidden sm:flex sm:flex-col sm:items-end sm:leading-tight">
                  ...
                  <div className="text-xs text-muted-foreground">Assistant • {user?.role}</div>
                </div>
                <Button
                  variant="outline"
                  size="sm"
